...
computeTagValue(int i, int j, int k, amrex::Array4<amrex::Real const> const& S_new_array)
{
    ...
    auto computeFlag =
        ...
        // 提取当前点及其周围点的总密度值（第一相密度+第二相密度）
        amrex::GpuArray<amrex::Real, 5> q = {
            S_new_array(i + di * (-2), j + dj * (-2), k + dk * (-2), AlphaRho1) + S_new_array(i + di * (-2), j + dj * (-2), k + dk * (-2), AlphaRho2),
            S_new_array(i + di * (-1), j + dj * (-1), k + dk * (-1), AlphaRho1) + S_new_array(i + di * (-1), j + dj * (-1), k + dk * (-1), AlphaRho2),
            S_new_array(i, j, k, AlphaRho1) + S_new_array(i, j, k, AlphaRho2),
            S_new_array(i + di * 1, j + dj * 1, k + dk * 1, AlphaRho1) + S_new_array(i + di * 1, j + dj * 1, k + dk * 1, AlphaRho2),
            S_new_array(i + di * 2, j + dj * 2, k + dk * 2, AlphaRho1) + S_new_array(i + di * 2, j + dj * 2, k + dk * 2, AlphaRho2)
        };
        // 使用TENO5格式计算标记
    ...
    if constexpr (BL_SPACEDIM >= 2) {
        res = res || computeFlag(0, 1, 0); // y方向
    }
    if constexpr (BL_SPACEDIM >= 3) {
 ...
 */
void AmrLevelCong::errorEst(TagBoxArray& tags,
    int /*clearval*/,
    int /*tagval*/,
    Real time,
    int /*n_error_buf*/,
    int /*ngrow*/)
{
    ...
    if (level < max_phigrad_lev || true) {
        ...
        Sborder.define(S_new.boxArray(), S_new.DistributionMap(), S_new.nComp(), 3); // 定义边界数组
        FillPatch(*this, Sborder, 2, cur_time, State_Type, 0, S_new.nComp()); // 填充补丁
    }
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
#endif
    {
        ...
        for (MFIter mfi(S_new, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
            ...
            Array4<Real const> const& Sborder_array = Sborder[mfi].const_array(); // 边界数组
            ...
            amrex::ParallelFor(tilebx,
                [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
                    // 计算标记值
                    bool flag = computeTagValue(i, j, k, Sborder_array);
                    if (flag) {
